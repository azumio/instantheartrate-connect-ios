...
-(void)measureHeartRate{
    ...
    NSURL *urlPro = [NSURL URLWithString:[NSString stringWithFormat:@"si.modula.instantheartrate:?callback=%@:?heartrate=!HR!",schema]];
                     
    NSURL *urlFree = [NSURL URLWithString:[NSString stringWithFormat:@"si.modula.instantheartrate.free:?callback=%@:?heartrate=!HR!",schema]];
    
    if( [[UIApplication sharedApplication] canOpenURL:urlFree] ){
        [[UIApplication sharedApplication] openURL:urlFree options:@{} completionHandler:^(BOOL success) {
            
        }];
    } else if( [[UIApplication sharedApplication] canOpenURL:urlPro] ){
        [[UIApplication sharedApplication] openURL:urlPro options:@{} completionHandler:^(BOOL success) {
...
-(void)alertView:(UIAlertView *)alertView clickedButtonAtIndex:(NSInteger)buttonIndex{
    NSString *installURL = [NSString stringWithFormat: @"https://api.azumio.com/partner/install/si.modula.instantheartrate.free?key=%@",self.partnerKey];
    
    if(buttonIndex != 0 ){
        [[UIApplication sharedApplication] openURL:[NSURL URLWithString:installURL] options:@{} completionHandler:^(BOOL success) {
